...
const getOrCreate = async (userId: string, defaults?: Partial<CreateUserSettingsInput>): Promise<UserSettings> => {
  ...
  return await create({
    ...
    preferredWorkStartTime: defaults?.preferredWorkStartTime,
    preferredWorkEndTime: defaults?.preferredWorkEndTime,
    minBreakBetweenMinutes: defaults?.minBreakBetweenMinutes,
