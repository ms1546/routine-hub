...
Routune Hub は、他人が設計した「習慣（Routine）」を再利用・カスタマイズし、
...
Routune Hubでは、**Clone（クローン）**と**Follow（フォロー）**を明確に区別しています。
...
Routune HubのAIは、**決して自動判断を行いません**。すべてのAI出力は「提案」として扱われ、人間の明示的な確認が必要です。
...
flowchart TB
  ...
  subgraph AWS[AWS ap-northeast-1]
    subgraph VPC[VPC (Public Subnets only)]
      ...
      ALB[ALB (internet-facing)\nSG: inbound none (手動で自宅IP/32追加)]
      subgraph AZA[Public Subnet A]
        ECSA[ECS Fargate Task\n(assign public IP)]
      end
      subgraph AZB[Public Subnet B]
        ECSB[ECS Fargate Task\n(assign public IP)]
      end
    end

    DDB[DynamoDB\n(user-settings / routines / ai-execution-logs)]
    SM[Secrets Manager (optional)]
    CW[CloudWatch Logs]
  end

  GH --> CI --> ECR[ECR Repository]
  GH --> TF --> AWS
...
1. GitHub Actions の `Deploy Production` が成功していることを確認
2. ワークフロー出力の `ALB DNS` / `App URL` を控える
3. ALBのセキュリティグループに自宅IP/32を追加
   - `curl https://checkip.amazonaws.com` で自宅IPを確認
   - `routune-hub-production-alb` のインバウンドに `80/443` を自宅IP/32で追加
4. ALB Target Group のヘルスチェックが `Healthy` になっていることを確認
5. `App URL` にアクセスしてトップページが表示されることを確認
6. CloudWatch Logs で ECS タスクのログが出力されていることを確認

※ セキュリティグループの手動変更は、次回 `terraform apply` で消えるため再設定が必要です。
